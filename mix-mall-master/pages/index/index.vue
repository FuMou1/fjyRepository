		...
		methods: {
			...
			async loadData() {
				...
				uni.request({
				      ...
				      url: `${this.$baseUrl}/app/api/mindexsetting/listByPage`,
				      ...
				      success: (res)=>{console.log("成功");console.log(res);
						var mindexsettings = res.data.data.records;
						
						for(var i=0;i<mindexsettings.length;i++){
							mindexsettings[i].miImage = "http://www.whycools.cn/Upload/"+mindexsettings[i].miImage;
						}
						this.carouselList = mindexsettings;
						
						this.titleNViewBackground = mindexsettings[0].background;
						this.swiperLength = mindexsettings.length;
					 },
			...
			navToDetailPage(item) {
				...
				let pNum = item.pNum;
				uni.navigateTo({
					url: `/pages/product/product?pNum=${pNum}`
				})
			},
		},
		// #ifndef MP
